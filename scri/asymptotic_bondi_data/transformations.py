...
def transform(self, **kwargs):
    ...
    if self.interpolant == None:
        ...
        self.interpolant = CubicSpline(self.u, np.array([sf.Grid(self.psi0.evaluate(grid_rotors), spin_weight=2),
                                               sf.Grid(self.psi1.evaluate(grid_rotors), spin_weight=1),
                                               sf.Grid(self.psi2.evaluate(grid_rotors), spin_weight=0),
                                               sf.Grid(self.psi3.evaluate(grid_rotors), spin_weight=-1),
                                               sf.Grid(self.psi4.evaluate(grid_rotors), spin_weight=-2),
                                               sf.Grid(self.sigma.evaluate(grid_rotors), spin_weight=2)]), axis=1)
    ...
    # ψ0(u', θ', ϕ') exp(2iλ)
    ψ0 = sf.Grid(sf.Modes(spinsfast.map2salm(f_of_timeprime[0], 2, working_ell_max), spin_weight=2).evaluate(distorted_grid_rotors), spin_weight=2)
    # ψ1(u', θ', ϕ') exp(iλ)
    ψ1 = sf.Grid(sf.Modes(spinsfast.map2salm(f_of_timeprime[1], 1, working_ell_max), spin_weight=1).evaluate(distorted_grid_rotors), spin_weight=1)
    # ψ2(u, θ', ϕ')
    ψ2 = sf.Grid(sf.Modes(spinsfast.map2salm(f_of_timeprime[2], 0, working_ell_max), spin_weight=0).evaluate(distorted_grid_rotors), spin_weight=0)
    # ψ3(u', θ', ϕ') exp(-1iλ)
    ψ3 = sf.Grid(sf.Modes(spinsfast.map2salm(f_of_timeprime[3], -1, working_ell_max), spin_weight=-1).evaluate(distorted_grid_rotors), spin_weight=-1)
    # ψ4(u', θ', ϕ') exp(-2iλ)
    ...
    #
    fprime_of_timeprime_directionprime = np.empty((6, timeprime.size, n_theta, n_phi), dtype=complex)
    # ψ0'(u, θ', ϕ')
    fprime_temp = ψ4.copy()
    ...
    fprime_of_timeprime_directionprime[0] = fprime_temp
    # ψ1'(u, θ', ϕ')
    fprime_temp = -ψ4
    fprime_temp *= ðuprime_over_k
    fprime_temp += 3 * ψ3
    ...
    fprime_temp += ψ1
    fprime_temp *= one_over_k_cubed
    fprime_of_timeprime_directionprime[1] = fprime_temp
    # ψ2'(u, θ', ϕ')
    fprime_temp = ψ4.copy()
    fprime_temp *= ðuprime_over_k
    fprime_temp += -2 * ψ3
    ...
    fprime_temp += ψ2
    fprime_temp *= one_over_k_cubed
    fprime_of_timeprime_directionprime[2] = fprime_temp
    # ψ3'(u, θ', ϕ')
    fprime_temp = -ψ4
    fprime_temp *= ðuprime_over_k
    fprime_temp += ψ3
    fprime_temp *= one_over_k_cubed
    fprime_of_timeprime_directionprime[3] = fprime_temp
    # ψ4'(u, θ', ϕ')
    fprime_temp = ψ4.copy()
    fprime_temp *= one_over_k_cubed
    fprime_of_timeprime_directionprime[4] = fprime_temp
    # σ'(u, θ', ϕ')
    fprime_temp = σ.copy()
    fprime_temp -= ððα
